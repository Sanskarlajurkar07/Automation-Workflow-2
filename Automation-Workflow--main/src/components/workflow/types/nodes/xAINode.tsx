...
const XAINode: React.FC<XAINodeProps> = ({ id, data, selected }) => {
  ...
  useEffect(() => {
    ...
    if (!data.params?.model) {
      updateNodeData(id, { model: 'grok-1' });
    }
    if (!data.params?.nodeName) {
      updateNodeData(id, { nodeName: `xai_${id.split('-')[1] || '0'}` });
    }
  ...
  return (
    <div className={`bg-white rounded-lg shadow-lg overflow-hidden ${
      ...
      <div className="bg-gradient-to-r from-blue-500 to-blue-600 px-4 py-3">
        <div className="flex items-center justify-between">
          <div className="flex items-center space-x-3">
            ...
            <div>
              <h3 className="font-medium text-white">{data.params?.nodeName || 'Grok AI'}</h3>
              <p className="text-xs text-blue-50/80">
                {data.params?.model || 'grok-1'}
              </p>
            </div>
          </div>
          <div className="flex items-center space-x-2">
            ...
            <button
              onClick={() => {
                if (window.confirm('Are you sure you want to delete this Grok AI node?')) {
                  removeNode(id);
                }
              }}
              ...
              <div className="space-y-1">
                ...
                <div className="relative">
                  <input
                    ...
                    placeholder="Enter your X.AI/Grok API Key"
                  />
                  <button
                    type="button"
                ...
                <p className="text-xs text-gray-500">
                  X.AI/Grok API access is currently limited. Get access at <a href="https://grok.x.ai" target="_blank" rel="noopener noreferrer" className="text-blue-500 hover:underline">grok.x.ai</a>
                </p>
              </div>
            </div>
          )}
      ...
      <div className="px-4 py-2 bg-blue-50 border-t border-blue-100 flex items-center justify-between text-xs text-blue-500">
        ...
        <span className="font-mono">Beta API</span>
      </div>
