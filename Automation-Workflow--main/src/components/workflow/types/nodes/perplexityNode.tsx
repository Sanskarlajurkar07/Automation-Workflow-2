...
import { 
  Settings, Trash2, Copy, LightbulbIcon, Eye, EyeOff, 
  AlertTriangle, MessageSquare, ChevronDown, ChevronUp,
  Zap, Sparkles, BookOpen, Globe
} from 'lucide-react';
...
// Updated model options - include newer models
const modelOptions = [
  'pplx-7b-online', 
  'pplx-70b-online', 
  'pplx-7b-chat',
...
const PerplexityNode: React.FC<PerplexityNodeProps> = ({ id, data, selected }) => {
  ...
  useEffect(() => {
    ...
    if (!data.params?.nodeName) {
      updateNodeData(id, { nodeName: `perplexity_${id.split('-')[1] || '0'}` });
    }
  ...
  return (
    <div className={`bg-white rounded-lg shadow-lg overflow-hidden ${
      selected ? 'ring-2 ring-rose-500' : hasErrors ? 'border border-red-300' : 'border border-gray-200'
    }`}>
      {/* Header */}
      <div className="bg-gradient-to-r from-rose-500 to-pink-600 px-4 py-3">
        <div className="flex items-center justify-between">
          <div className="flex items-center space-x-3">
            ...
            <div>
              <h3 className="font-medium text-white">{data.params?.nodeName || 'Perplexity'}</h3>
              <p className="text-xs text-pink-50/80">
                {data.params?.model || 'sonar-small-online'}
              </p>
            </div>
          </div>
          <div className="flex items-center space-x-2">
            ...
            <button
              onClick={() => {
                if (window.confirm('Are you sure you want to delete this Perplexity node?')) {
                  removeNode(id);
                }
              }}
      ...
      <div className="p-4 space-y-5">
        ...
        <div className="space-y-2">
          ...
          <input
            ...
            className="w-full px-3 py-2 text-sm bg-gray-50 border border-gray-200 rounded-lg focus:ring-2 focus:ring-rose-500 focus:border-transparent"
          />
        </div>
        ...
        <div className="space-y-2">
          <label className="text-xs font-medium text-gray-500 flex items-center">
            ...
            <span className="ml-2 px-1.5 py-0.5 text-xs rounded-md bg-rose-100 text-rose-800">
        ...
        <div className="space-y-2">
          ...
          {expandedSections.model && (
            <div className="pt-2 space-y-3">
              <select
                className="w-full rounded-md border-gray-300 shadow-sm focus:border-rose-500 focus:ring-rose-500 sm:text-sm"
        ...
        <div className="space-y-2">
          ...
          >
            <label className="block text-sm font-medium text-gray-700 flex items-center">
              <Settings className="w-4 h-4 mr-2 text-rose-500" />
            ...
            <div className="pt-2 space-y-4">
              ...
              <div className="space-y-1">
                ...
                <p className="text-xs text-gray-500">
                  Get your API key from <a href="https://www.perplexity.ai/api" target="_blank" rel="noopener noreferrer" className="text-rose-500 hover:underline">perplexity.ai/api</a>
                </p>
              </div>
            </div>
          )}
      ...
      <Handle
        ...
        className="w-3 h-3 -mr-0.5 bg-rose-500 border-2 border-white rounded-full"
        style={{ top: '50%' }}
      />
    </div>
  );
